...
class ComplaintViewSet(ModelViewSet):
  ...
  def list(self, request):
    ...
    filtered_queryset = Complaint.objects.filter(account="NYCC{district}".format(district=user.district))
...
class ConstituentsComplaintViewSet(ModelViewSet):
  http_method_names = ['get']
  def list(self, request):
      user = UserProfile.objects.get(pk=request.user.id)
      filtered_queryset = Complaint.objects.filter(council_dist="NYCC{district}".format(district=user.district))
      serializer = ComplaintSerializer(filtered_queryset, many=True)
      return Response(serializer.data)